...
app.post("/ingest_single", async (req, res) => {
  try {
    ...
    const indexResponse = await esClient.index({
      index: index, // Update with your desired Elasticsearch index name
      body: logData,
    });
...
// Bulk log ingestion API endpoint
app.post("/ingest_bulk", async (req, res) => {
  try {
    ...
    const body = logsData.flatMap((log) => [{ index: { _index: index } }, log]);
    ...
    if (bulkResponse.errors) {
      ...
      return res
        .status(500)
        .json({ status: "error", message: "Error ingesting logs in bulk" });
    }
